...
부서 데이터는 본질적으로 **트리(계층) 구조**입니다. A사 > DEV > 웹개발팀처럼 상위-하위 관계가 존재하고, 이 관계를 기반으로 조직도를 렌더링하거나, 특정 부서의 하위 부서를 모두 조회하거나, 메달 지급 대상자를 찾는 등의 기능이 필요합니다.
...
**Before — parentId 없음: 관계를 알 수 없는 플랫 구조**

```mermaid
graph TD
    A1["A사"] ~~~ B1["개발본부"] ~~~ C1["Web팀"]
    D1["App팀"] ~~~ E1["경영지원"] ~~~ F1["인사팀"]
...
**After — parentId + depth 도입: 명시적 트리 구조**

```mermaid
graph TD
    A["A사 · depth=1"] --> B["개발본부 · depth=2"]
    A --> E["경영지원 · depth=2"]
    B --> C["Web팀 · depth=3"]
    B --> D["App팀 · depth=3"]
    E --> F["인사팀 · depth=3"]
...
> Wave 1 (파랑): `[A사]` / Wave 2 (보라): `[개발본부, 경영지원]` / Wave 3 (주황): `[Web팀, App팀, 인사팀]` — 각 Wave = DB 쿼리 1회
...
```mermaid
flowchart TD
    A["1. 권한 부서 조회"] -->|"2개 부서"| B["2. BFS 하위부서 확장"]
    B -->|"6개 부서"| C["3. DeptEmployee 조회"]
    C -->|"사원 목록"| D["4. empSeq 추출"]
    D --> E["5. 상세 정보 + 코드→한글"]
    E --> F["Page 응답 반환"]
