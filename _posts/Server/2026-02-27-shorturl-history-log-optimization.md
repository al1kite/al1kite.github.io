...
### 왜 MongoDB 범위 쿼리가 아니라 인메모리 이진 탐색인가

여기서 한 가지 의문이 생길 수 있습니다. MongoDB에 `startIntegerIp` 인덱스가 이미 있으니, 굳이 20만 건을 메모리에 올려서 직접 이진 탐색을 구현할 필요 없이 **MongoDB 범위 쿼리**로 해결할 수도 있지 않을까?

```javascript
// MongoDB 범위 쿼리로 국가 찾기
db.countryIp.findOne({
  startIntegerIp: { $lte: ipNumber },
  endIntegerIp: { $gte: ipNumber }
})
```

`startIntegerIp: -1` 인덱스가 있으면 `$lte` 조건까지는 B-Tree 인덱스 스캔으로 후보를 빠르게 좁히고, 이후 `endIntegerIp >= ipNumber` 조건은 후보 Document를 가져와 필터링합니다. 쿼리 자체의 실행 시간은 밀리초 단위로 빠릅니다.

**하지만 문제는 쿼리의 속도가 아니라, 이 쿼리를 몇 번 호출하느냐**입니다.

#### 네트워크 왕복 비용: 쿼리 1회 vs 1,000회

```
[MongoDB 범위 쿼리 방식]
배치: 로그 1,000건 처리
  → IP 1개당 MongoDB 쿼리 1번
  → 1,000번의 네트워크 왕복
  → 각 왕복마다:
      커넥션 풀에서 커넥션 할당
      → Java 객체 → BSON 직렬화
      → TCP 전송 (localhost여도 소켓 통신)
      → MongoDB 쿼리 엔진 실행
      → BSON 응답 → Java 객체 역직렬화
      → 커넥션 반환
  → 쿼리 자체가 1ms여도 오버헤드 포함 시 쿼리당 ~2~3ms
  → 1,000건: ~2~3초
  → 10,000건: ~20~30초

[인메모리 이진 탐색 방식]
배치: 로그 1,000건 처리
  → 캐시 워밍 1회 (서버 시작 시, 이후 JVM 힙에 상주)
  → IP 1개당 ArrayList.get(mid) 호출 ~18회
  → 네트워크 왕복 0번
  → 1,000건 전체가 수 밀리초 이내
  → 10,000건이어도 수십 밀리초 이내
```

인덱스가 아무리 빨라도 **네트워크 왕복 비용은 제거할 수 없습니다.** 같은 서버에서 localhost MongoDB를 호출하더라도 쿼리 한 번에 커넥션 풀 할당, BSON 직렬화/역직렬화, MongoDB Wire Protocol 처리, TCP 소켓 통신 등 수백 마이크로초~수 밀리초의 고정 오버헤드가 붙습니다. 이 오버헤드는 데이터 크기가 아니라 **호출 횟수에 비례**하므로, 조회 대상이 1건이든 1만 건이든 매번 동일하게 발생합니다.

반면 인메모리 이진 탐색은 같은 JVM 힙 안에서 `ArrayList.get(index)` 호출, 즉 배열의 메모리 주소를 직접 참조하는 연산만 수행합니다. 네트워크 I/O, 직렬화, 프로토콜 처리가 모두 없으므로 단일 조회가 **나노초 단위**에서 끝납니다.

#### 응답 시간의 안정성(Latency Consistency)

성능 차이는 평균 속도뿐 아니라 **편차**에서도 나타납니다.

MongoDB 쿼리는 아무리 빨라도 외부 프로세스 호출입니다. GC(Garbage Collection), WiredTiger 캐시 eviction, 디스크 I/O, 다른 쿼리와의 Lock 경합 등에 의해 특정 쿼리만 수십 밀리초로 튀는 경우가 발생할 수 있습니다. 1,000번 호출하면 그중 몇 번은 느려지는 것이 자연스럽고, 이런 꼬리 지연(tail latency)이 배치 전체 처리 시간을 끌어올립니다.

인메모리 이진 탐색은 JVM 내부 연산이므로 편차가 거의 없습니다. 20만 건에서 최악의 경우에도 정확히 18번의 비교로 결과가 나옵니다.

#### 커넥션 풀 점유와 다른 작업에 대한 영향

MongoDB 드라이버의 커넥션 풀은 기본적으로 **maxPoolSize = 100**입니다. 배치가 실행되는 동안 IP 변환을 위해 1,000번의 쿼리를 연속으로 발생시키면, 그 시간 동안 커넥션을 지속적으로 점유합니다.

이 배치와 같은 시간에 실행되는 다른 작업들이 있습니다:
- 증분 로그 조회 (`findAllByCallDateBetween`)
- 기존 집계 데이터 로드 (`findAllBy_idIn`)
- 집계 결과 저장 (`saveAll`)

이 작업들도 커넥션이 필요합니다. IP 변환이 커넥션을 대량으로 점유하면, 나머지 작업이 커넥션을 확보하지 못해 대기하게 되고, 배치 전체 처리 시간이 길어집니다.

인메모리 이진 탐색은 MongoDB 커넥션을 전혀 사용하지 않으므로, 커넥션 풀의 여유를 다른 작업에 온전히 남겨둘 수 있습니다.

#### 이 선택이 성립하는 전제 조건

인메모리 캐시 + 이진 탐색이 MongoDB 범위 쿼리보다 유리한 것은, **CountryIp 데이터가 캐싱에 이상적인 특성**을 가지고 있기 때문입니다:

| 특성 | 값 | 캐싱 적합성 |
|---|---|---|
| 변경 빈도 | 월 1회 이하 (IP2Location 갱신 주기) | 캐시 무효화가 거의 불필요 |
| 데이터 크기 | 20만 건 ≈ 35MB | JVM 힙(256MB~2GB)에 부담 없음 |
| 조회 빈도 | 배치마다 로그 수만큼 반복 조회 | 한 번 캐시하면 적중률 100% |

세 조건이 모두 충족되지 않으면 MongoDB 범위 쿼리가 더 나은 선택입니다:

- **데이터가 실시간으로 변한다면**: 캐시와 원본 사이의 불일치가 발생합니다. 캐시를 자주 갱신해야 하고, 그때마다 20만 건을 다시 로드하는 비용이 발생합니다.
- **데이터가 수천만 건이라면**: 메모리에 올릴 수 없습니다. 수천만 건 × 168 bytes ≈ 수 GB가 되어 JVM 힙을 압박하고, GC 비용도 급격히 증가합니다. 이 경우 MongoDB 인덱스에 맡기는 것이 올바른 선택입니다.
- **조회 빈도가 낮다면**: 요청당 1~2번만 조회한다면 네트워크 왕복 비용이 무시할 수 있는 수준이고, 35MB를 메모리에 상주시키는 것이 오히려 낭비입니다.

이 프로젝트에서는 세 조건이 모두 충족되므로, 인메모리 캐시 + 이진 탐색이 최적의 선택이었습니다.

#### MongoDB 인덱스와 이진 탐색의 역할 정리

둘은 **같은 문제를 다른 계층에서 해결하는 것이 아니라, 각각 다른 문제를 담당**합니다:

| | MongoDB `startIntegerIp` 인덱스 | 인메모리 이진 탐색 |
|---|---|---|
| **해결하는 문제** | 캐시 워밍 시 20만 건을 정렬된 채로 빠르게 로드 | 배치 처리 시 IP → 국가를 네트워크 없이 조회 |
| **실행 시점** | 서버 기동 시 1회 | 배치 실행 시 로그마다 |
| **실행 빈도** | 극히 낮음 | 매우 높음 (로그 수만큼) |
| **없어도 되는가** | Java에서 `Collections.sort()`로 대체 가능 | 대체 불가 — 핵심 성능 요소 |

MongoDB 인덱스가 없어도 `findAll()`로 비정렬 로드 후 Java에서 정렬하면 됩니다. 20만 건의 Java 정렬은 수십 밀리초면 끝나고, 서버 기동 시 한 번만 실행되므로 체감 차이가 없습니다. 다만 인덱스가 있으면 MongoDB의 100MB 메모리 정렬 제한에 대한 우려 없이 안전하게 동작하고, 워밍 시간도 약간 줄어들므로 걸어두는 것이 좋습니다.

반면 인메모리 이진 탐색은 대체할 수 없습니다. 이것이 없으면 IP → 국가 변환을 MongoDB 쿼리로 해야 하고, 앞서 분석한 대로 네트워크 왕복 비용이 배치 처리 시간을 수십 배로 늘립니다. **이진 탐색이 이 최적화의 진짜 성능을 만들어내는 핵심**이고, MongoDB 인덱스는 그 이진 탐색이 동작할 수 있도록 정렬된 리스트를 준비하는 보조 역할입니다.
...
이 인덱스의 역할은 **캐시 워밍 시 정렬된 데이터를 효율적으로 로드하는 것**에 한정됩니다. 실제 IP → 국가 매핑의 성능을 만들어내는 것은 인메모리 이진 탐색이고, 이 인덱스는 그 이진 탐색이 동작할 수 있도록 정렬된 리스트를 준비하는 사전 작업을 빠르게 해줄 뿐입니다. 앱 기동 시 한 번만 사용되지만, 워밍 시간이 길어지면 서비스 시작이 지연되므로 걸어두는 것이 좋습니다.
...
| 컬렉션 | 필드 | 타입 | 용도 |
|---|---|---|---|
| `countryIp` | `startIntegerIp` | 내림차순(-1) | 캐시 워밍 시 정렬된 로드 (이진 탐색 사전 준비) |
