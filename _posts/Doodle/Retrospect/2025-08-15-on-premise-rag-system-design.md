...
주식 시장 분석처럼 **정확성과 최신성이 동시에 요구되는 도메인**에서 RAG는 거의 필수적입니다. SK하이닉스 최근 이슈를 물었을 때, 6개월 전 학습 데이터가 아니라 오늘 수집된 뉴스를 기반으로 답변해야 하니까요.
...
**2. 벡터 데이터베이스(Vector DB)** — 임베딩된 벡터를 저장하고, 유사도 검색을 수행합니다. 이 질문과 가장 비슷한 문서 5개를 찾아줘라는 요청을 밀리초 단위로 처리합니다.
...
n8n에서는 하나의 노드 출력을 여러 노드에 연결하면 **자동으로 병렬 실행**됩니다. Python에서 `asyncio.gather()`나 `ThreadPoolExecutor`로 구현할 수 있지만, 시각적으로 이 세 API가 동시에 호출된다는 것이 즉각 드러나는 건 n8n의 장점입니다.

#### 3. RAG의 두뇌는 코드로, 손발은 워크플로로
...
공모전 팀에 비개발자 팀원이 있었습니다. 뉴스 수집 소스를 추가하고 싶다거나 DART 대신 다른 공시 API를 써보자라는 요청이 올 때, n8n 워크플로에서 노드 하나를 추가하는 것과 Python 코드를 수정하는 것의 진입 장벽은 다릅니다.
...
- **`responseMode: whenLastNodeFinished`**: 워크플로 전체가 끝날 때까지 기다렸다가 응답. lastNodeExecuted로 하면 다음 노드 실행 즉시 응답.
...
3. **한국어 특화**: Exaone은 LG AI연구원이 한국어 코퍼스로 집중 학습한 모델. 실적이 컨센서스를 상회같은 금융 표현도 잘 이해합니다.
...
2. **도메인 특화**: 실적이 시장 기대를 상회는 긍정, 조정 압력이 강화는 부정 — 금융 도메인의 감정 판단은 일반 감정 분석과 다릅니다.
...
뉴스 중에는 SK하이닉스 채용박람회 개최처럼 주가와 무관한 PR성 기사도 많습니다. 이를 필터링하기 위해 키워드 기반 시장 관련도 점수를 계산합니다.
...
- **`score_threshold=0.3`**: 벡터 유사도가 0.3 미만인 문서는 제외. 너무 낮으면 삼성전자를 검색했는데 삼성생명 문서가 섞일 수 있음.
...
3. **할루시네이션 방지**: context에 없는 내용을 지어내지 않는다, 정확한 수치를 사용한다를 명시. 로컬 모델은 GPT-4보다 할루시네이션 경향이 강해서 이런 제약이 필수입니다.
...
텍스트 임베딩은 **COSINE**이 표준입니다. 삼성전자 HBM 양산과 삼성전자 HBM3E 출하 같은 문장은 방향(의미)이 비슷하지만, 길이(단어 수)는 다를 수 있습니다. 코사인은 길이를 정규화하므로 의미적 유사성을 잘 포착합니다.
...
- `KEYWORD`: 정확한 일치 검색. 삼성전자로 필터링하면 삼성은 안 나옴.
...
→ 감정 과열도가 상이므로 신중함을 강조하면서, 이전 대화 맥락을 고려한 피드백을 생성합니다.
...
벡터 유사도만으로는 **의미적 유사성**만 측정합니다. SK하이닉스가 문서에 많이 나오면 유사도가 높지만, **실제 질문과의 관련성**은 낮을 수 있습니다.
...
RAG는 결국 검색 + 생성이라는 단순한 조합이지만, 실제로 시스템을 구축하면 **수집 → 정제 → 청킹 → 임베딩 → 저장 → 검색 → 프롬프트 구성 → 생성 → 파싱**이라는 긴 파이프라인을 관리해야 합니다.
...
완벽한 시스템은 아닙니다. 200건짜리 감정 분류 모델, 인메모리 세션, 단일 임베딩 모델 — 개선할 점이 많습니다. 하지만 작게 시작하고, 동작하는 것부터 만들자는 원칙으로 실제로 돌아가는 온프레미스 RAG 시스템을 완성할 수 있었고, 코스콤 공모전 1차 심사도 통과할 수 있었습니다.
...
마지막으로 당시 만들었던 발표자료 첨부하며 글을 마무리하겠습니다.

[![9b4d62e4-01.png](https://i.postimg.cc/QCQxhVv4/9b4d62e4-01.png)](https://postimg.cc/5QtMm94L)
[![9b4d62e4-02.png](https://i.postimg.cc/dVttcs6X/9b4d62e4-02.png)](https://postimg.cc/TpFXrXZq)
[![9b4d62e4-03.png](https://i.postimg.cc/brhpCpdZ/9b4d62e4-03.png)](https://postimg.cc/hzpkhFLn)
[![9b4d62e4-04.png](https://i.postimg.cc/TYcYsLVp/9b4d62e4-04.png)](https://postimg.cc/RNNB60bB)
[![9b4d62e4-05.png](https://i.postimg.cc/15MsBxS9/9b4d62e4-05.png)](https://postimg.cc/Lnqc4bh7)
[![9b4d62e4-06.png](https://i.postimg.cc/7P3xCGQ1/9b4d62e4-06.png)](https://postimg.cc/F1Kt8Rv1)
[![9b4d62e4-07.png](https://i.postimg.cc/NfbBDhtK/9b4d62e4-07.png)](https://postimg.cc/crKp1brW)
[![9b4d62e4-08.png](https://i.postimg.cc/YC9kyXqK/9b4d62e4-08.png)](https://postimg.cc/PPBBCQw2)
[![9b4d62e4-09.png](https://i.postimg.cc/nhzZwdVk/9b4d62e4-09.png)](https://postimg.cc/6yDD8fv2)
[![9b4d62e4_10.png](https://i.postimg.cc/907JLwr0/9b4d62e4_10.png)](https://postimg.cc/1VyGtt01)
[![9b4d62e4_11.png](https://i.postimg.cc/h4zyD6Mw/9b4d62e4_11.png)](https://postimg.cc/xqQygswy)
[![9b4d62e4-12.png](https://i.postimg.cc/153L98xn/9b4d62e4-12.png)](https://postimg.cc/gr1Msk2m)
[![9b4d62e4-13.png](https://i.postimg.cc/L68rHJdg/9b4d62e4-13.png)](https://postimg.cc/kDLjsXg7)
[![9b4d62e4-15.png](https://i.postimg.cc/HskfprGL/9b4d62e4-15.png)](https://postimg.cc/LhGyD68d)
[![9b4d62e4-16.png](https://i.postimg.cc/65pD98sd/9b4d62e4-16.png)](https://postimg.cc/23gHG8S6)
[![9b4d62e4-17.png](https://i.postimg.cc/gkt1rGdR/9b4d62e4-17.png)](https://postimg.cc/nC73PbXc)
[![9b4d62e4-18.png](https://i.postimg.cc/bNV4dq81/9b4d62e4-18.png)](https://postimg.cc/VdMKVQJN)
[![9b4d62e4-19.png](https://i.postimg.cc/CLr9dwYG/9b4d62e4-19.png)](https://postimg.cc/3dptc5kN)
[![9b4d62e4-20.png](https://i.postimg.cc/RVp2qv9L/9b4d62e4-20.png)](https://postimg.cc/nC73PbXX)
